...
- Added consistent qubit index assignment for tiling templates
  - `get_data_indices_cube()` method for cube patches with patch coordinate-based consistent indexing
  - `get_data_indices_pipe()` method for pipe patches with boundary-based consistent indexing
  - `calculate_qindex_base_cube()` function ensuring patches at same coordinates get same qubit indices
  - `calculate_qindex_base_pipe()` function ensuring pipes sharing same physical qubits get same indices
  - Enables stable mapping across different temporal layers
