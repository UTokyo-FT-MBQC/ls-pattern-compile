...
valid_node_ids: set[int] = set(
    int(n) for n in getattr(compiled_canvas.global_graph, "physical_nodes", []) or []
)
if not valid_node_ids:
    valid_node_ids = {int(n) for n in compiled_canvas.coord2node.values()}
nnodes = (
...
def extract_backward_sensitivity_nodes(
    error: Exception, *, valid_nodes: set[int]
) -> list[int]:
    """
    Parse stim's backward sensitivity message into node ids present on the canvas,
    filtering to anti-commuting Paulis relative to the reported error basis.

    Stim reports non-deterministic observables with a section like::

        The backward-propagating error sensitivity for L0 was:
            Z0
            Z3

    This helper pulls the integer suffixes out of that section and filters them
    against the compiled canvas node set so we can forward them to the visualizer.
    If the error is X-basis we keep only Z-sensitivity terms (and vice versa).
    """
    message = str(error)
    ...
    error_basis = basis_match.group(1).upper() if basis_match else None
    pauli_filter = {"X": "Z", "Z": "X"}.get(error_basis)

    pattern = re.compile(
        r"backward[-\s]propagating error sensitivity for .*?:\s*(.*?)(?:\n\s*\n|\Z)",
    ...
    for match in pattern.finditer(message):
        ...
        for line in block.splitlines():
            ...
            if not token:
                continue
            if pauli_filter and not token.upper().startswith(pauli_filter):
                continue
            num_match = re.search(r"(-?\d+)", token)
            if not num_match:
                continue
            node_id = int(num_match.group(1))
...
backward_sensitivity_nodes: list[int] = []
dem = None
try:
    ...
    print(err)
except Exception as exc:  # stim reports non-deterministic observables here
    print(f"Error during DEM creation or matching: {exc}")
    backward_sensitivity_nodes = extract_backward_sensitivity_nodes(
        exc, valid_nodes=valid_node_ids
    )
    if backward_sensitivity_nodes:
        print(f"Backward sensitivity nodes: {backward_sensitivity_nodes}")
    else:
        print("No backward sensitivity nodes extracted from error message.")
else:
    svg = dem.diagram(type="match-graph-svg") if dem is not None else None
    if svg is not None:
        try:
            ...
            print(f"Could not write SVG diagram: {exc}")

fig3d = visualize_compiled_canvas_plotly(
    compiled_canvas,
    show_edges=True,
    show_xparity=True,
    hilight_nodes=backward_sensitivity_nodes or None,
)
fig3d.show()
